...
exports.run = (client, message, args) => {if(db.fetch(`bakim`)) return message.channel.send('Şuanda Bakım Modu Açık. Komutlar Bakım Modunda Çalışmaz')
    ...
    .setAuthor(`${client.user.username} `, client.user.avatarURL) 
      ...
      .addField('** Moderasyon (19)**',`Yetkililer İçin Moderasyon Komutları Bölümü . \n` +  '`küfür-engel`, `reklam-engelle`, `kick`,`mute`,`sil(gelişmiş)`, `temizle`, `votekick`, `kilit(kanal kilitler)`, `duyuru`, `ban`, `unban`, `anket`, `banlananlar`, `yavaşmod`, `ıd`, `reklam-kick`, `hazır-kanallar`, `panel`, `forceban`, `softban`, `tempmute`, `hgbb-ayarla`')
