...
exports.run = async (client, message, args) => {
  try {
    ...
    if (query < 1)
      return message.channel.send("Aramam için kelime girmen gerek");
    ...
    if (!body.response.docs.length)
      return message.say("Hiçbir sonuç bulunamadı.");
    const data =
      body.response.docs[Math.floor(Math.random() * body.response.docs.length)];
    const embed = new Discord.RichEmbed();
    embed.setColor(0x00a2e8);
    embed.setAuthor(
      "New York Times",
      "https://i.imgur.com/ZbuTWwO.png",
